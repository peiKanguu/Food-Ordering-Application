...
public:
	...
	void loadRidersFromFile(const string& filename) {
		ifstream inFile(filename);
		string line;
		getline(inFile, line); // Skip header line
	...
	void confirmation(Order* newOrder) {
		...
		if (confirmationChoice == 'y' || confirmationChoice == 'Y') {
			...
			//
			outFile << newOrder->getOrderID() << "," << newOrder->getRestaurantName() << "," << newOrder->getTotalPrice_food() << "," << newOrder->getDelivery().getDeliveryOption() << "," << newOrder->getPaymentMethod() << endl;
			outFile.close();
		}
		else {
	...
	void newOrder(vector<Restaurant*>& rest) {
		...
		loadRidersFromFile("RiderInformation.csv");
